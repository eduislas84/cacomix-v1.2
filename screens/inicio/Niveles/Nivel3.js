...
const openQuestions = [
  { id: 1, question: '¿Cómo se dice "Montaña" en náhuatl?', answer: 'Tepētl' },
  { id: 2, question: '¿Cómo se dice "Negro" en náhuatl?', answer: 'Yohualli' },
  { id: 3, question: '¿Cómo se dice "Templo" en náhuatl?', answer: 'Teocalli' },
  { id: 4, question: '¿Cómo se dice "Dios de la lluvia" en náhuatl?', answer: 'Tlāloc' },
  { id: 5, question: '¿Cómo se dice "Flor" en náhuatl?', answer: 'Xochitl' },
];
...
export default function Nivel3({ navigation }) {
  ...
  const handleNextQuestion = (userAnswer) => {
    ...
    if (userAnswer.trim().toLowerCase() === correctAnswer) {
      ...
      if (currentQuestionIndex < openQuestions.length + multipleChoiceQuestions.length - 1) {
        ...
        Alert.alert('¡Felicidades!', `Has completado el Nivel 3 con una puntuación de ${score + 1}/${openQuestions.length + multipleChoiceQuestions.length}.`, [
          { text: 'OK', onPress: () => navigation.navigate('Niveles', { level3Completed: true }) },
        ]);
      }
    } else {
